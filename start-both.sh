...
if [ -f "$CERT" ] && [ -f "$KEY" ]; then
    echo "Starting prod server (HTTPS) on https://0.0.0.0:8080"
    cd "$PROJECT_ROOT/backend"
    ...
    echo ""
    echo " PROD (phone testing — HTTPS)"
    echo "   Local:    https://localhost:8080"
    echo "   Network:  https://${LAN_IP}:8080"
    echo "========================================="
else
    echo "Starting prod server (HTTP) on http://0.0.0.0:8080"
    echo "Warning: PWA standalone mode requires HTTPS. See README."
    cd "$PROJECT_ROOT/backend"
    uvicorn training_status.api:app --host 0.0.0.0 --port 8080 --app-dir src &
    PROD_PID=$!
    echo ""
    ...
    echo "   Frontend: http://localhost:5173"
    echo ""
    echo " PROD (phone testing — HTTP only)"
