...
def calculate_consistency_score(volumes: list[float], rest_days: list[int], 
                                monotony_values: list[float]) -> dict:
...
def get_recommendation(tsb: Optional[float], hrv: Optional[float], 
                       resting_hr: Optional[int], sleep_score: Optional[float],
                       fatigue: Optional[int]) -> dict:
    ...
    if tsb is not None and tsb < -20:
        return {
            ...
            "reason": f"TSB is {tsb:.1f} - you're overreaching. Take a rest day or very easy recovery run.",
            "urgency": "high",
            "color": "red"
        }
...
def calculate_projections(ctl: float, atl: float, ramp_rate: Optional[float]) -> list[dict]:
    ...
    for day in range(1, 8):
        ...
        projections.append({
            ...
            "zone": (
                "Optimal" if proj_tsb > 5 else
                "Grey" if proj_tsb > -10 else
                "Overreach"
            )
        })
    return projections
...
def calculate_race_predictions(cs: float, d_prime: Optional[float], 
                                ctl: Optional[float], avg_pace: Optional[str]) -> list[dict]:
